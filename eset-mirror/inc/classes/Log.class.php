...
class Log
{
    ...
    static public function informer($str, $ver, $level = 0)
    {
        ...
        if (static::$CONF['MAILER']['log_level'] >= $level)
            static::$mailer_log .= sprintf("[%s] [%s] %s%s", date("d.m.Y"), date("H:i:s"), ($ver ? '[ver. ' . strval($ver) . '] ' : ''), $str) . chr(10);
    }
    ...
    static public function write_log($text, $level, $version = null, $ignore_rotate = false)
    {
        ...
        if ($level == 1) {
            ...
            $text = sprintf("[%s] %s%s", date("d.m.Y H:i:s"), ($version ? '[ver. ' . strval($version) . '] ' : ''), $text);
