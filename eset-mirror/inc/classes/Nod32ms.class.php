...
class Nod32ms
{
    ...
    private function generate_html()
    {
        ...
        if (Config::get('SCRIPT')['generate_only_table'] == '0') {
            ...
            $html_page .= '<style type="text/css">html,body{height:100%;margin:0;padding:0;width:100%}table#center{border:0;height:100%;width:100%}table td table td{text-align:center;color:white;vertical-align:middle;font-weight:bold;padding:10px 15px;border:0}table tr:nth-child(odd){background:#3b3b3b}table tr:nth-child(even){background:#0096a1}</style>';
            $html_page .= '</head>';
            $html_page .= '<body>';
        ...
        foreach ($DIRECTORIES as $ver => $dir) {
            if (Config::upd_version_is_set($ver) == '1') {
                ...
                $html_page .= '<td>' . ($timestamp ? date("d.m.Y H:i:s", $timestamp) : Language::t("n/a")) . '</td>';
                $html_page .= '</tr>';
            }
        }
        ...
        $html_page .= '<td colspan="2">' . (static::$start_time ? date("d.m.Y H:i:s", static::$start_time) : Language::t("n/a")) . '</td>';
        $html_page .= '</tr>';
